<template>
    <div class="radio-list" :disabled="disabled">
        <label v-for="item in items" :class="[{'is-checked': isItemChecked(item)}, getInputStyleClass]">
            <input
                type="radio"
                :disabled="disabled"
                :name="id"
            ...
            <span class="label-body">{{ getItemName(item) }}</span>
</template>
...
<script>
	...
	export default {
		...
		computed: {
            ...
            getInputStyleClass() {
                return this.schema.inputStyleClasses;
            }
		},
...
<style lang="sass">
	.vue-form-generator .field-radioSkel {
